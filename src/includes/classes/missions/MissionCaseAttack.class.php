 ...
 * @info $Id: MissionCaseAttack.class.php 2803 2013-10-06 22:23:27Z slaver7 $
 * @link http://2moons.cc/
 */
...
class MissionCaseAttack extends AbstractMission
{
	public function arrivalEndTargetEvent()
	{
		...
		{
			$combatData['fleet']['attack']['elements'][$fleetId]	= $fleetDetail['elements'][Vars::CLASS_FLEET];
			$combatData['fleet']['attack']['player']				= $fleetDetail['fleet_owner'];
			...
			$attackBonus	= (1 + (0.1 * $userData[Vars::getElement(109)->name]));
			$defenseBonus	= (1 + (0.1 * $userData[Vars::getElement(110)->name]));
			$shieldBonus	= (1 + (0.1 * $userData[Vars::getElement(111)->name]));

			$combatData['player'][$userData['id']]	= array(
				'attackBonus'	=> PlayerUtil::getBonusValue($attackBonus, 'Attack', $userData),
				'defenseBonus'	=> PlayerUtil::getBonusValue($defenseBonus, 'Defensive', $userData),
				'shieldBonus'	=> PlayerUtil::getBonusValue($shieldBonus, 'Shield', $userData),
			);
		}

		$combatData['player'][$targetUser['id']]	= $targetUser;
		...
		foreach($targetElements as $elementId => $elementObj)
		{
			...
			$combatData['fleet']['defender'][0]['elements'][$elementId] = $targetPlanet[$elementObj->name];
		}
		...
		foreach($fleetList as $fleetId => $fleetDetail)
		{
			$combatData['fleet']['defender']['elements'][$fleetId]	= $fleetDetail['elements'][Vars::CLASS_FLEET];
			$combatData['fleet']['defender']['player']				= $fleetDetail['fleet_owner'];
			...
			$attackBonus	= (1 + (0.1 * $userData[Vars::getElement(109)->name])); //attaque
			$defenseBonus	= (1 + (0.1 * $userData[Vars::getElement(110)->name])); //bouclier
			$shieldBonus	= (1 + (0.1 * $userData[Vars::getElement(111)->name])); //coque
		...
		unset($fleetList, $targetFleetsResult);

		require_once 'includes/classes/battle-engine/BattleEngineInterface.interface.php';

		$battleObjName = ucfirst(BATTLE_ENGINE).'BattleEngine';
		$battleObjPath = 'includes/classes/battle-engine/'.$battleObjName.'.class.php';

		require_once $battleObjPath;

		/** @var $battleObj BattleEngineInterface */
		$battleObj	= new $battleObjName;

		$battleObj->setConfig(array(
			'fleetIntoDebris'	=> $config->Fleet_Cdr,
			'defenseIntoDebris'	=> $config->Defs_Cdr,
			'maxRounds'			=> MAX_ATTACK_ROUNDS
		));

		$battleObj->setData($combatData);
		$battleObj->simulate($combatData);
		$combatResult 		= $battleObj->getResult();

		$battleObj	= NULL;

		unset($combatData, $battleObj);
