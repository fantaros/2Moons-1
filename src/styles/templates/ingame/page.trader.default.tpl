				...
				<div class="inner">
					...
					<div class="trader_col">
						{if !$requiredDarkMatter}
                            ...
                            <input type="image" id="trader_{$elementObj->name}" src="{$themePath}images/{$elementObj->name}.gif" title="{$LNG.tech.$elementId}" height="32" width="52"><br>
                            <label for="trader_{$elementObj->name}">{$LNG.tech.$elementId}</label>
                            </form>
						{else}
                            <img src="{$themePath}images/{$elementObj->name}.gif" title="{$LNG.tech.$elementId}" border="0" height="32" width="52" style="margin: 3px;"><br>{$LNG.tech.$elementId}
                        {/if}
					</div>
					{/foreach}
				</div>
