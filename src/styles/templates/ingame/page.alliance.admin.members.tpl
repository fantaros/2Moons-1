...
<table id="memberList" style="width:760px;" class="tablesorter">
	...
	<tr>
		...
		<td><a href="#" onclick="return Dialog.PM({$userID});"><img src="{$themePath}img/m.gif" border="0" title="{$LNG.write_message}"></a></td>
		...
		<td>{if $memberListRow.rankID != -1}<img src="{$themePath}pic/key.gif" border="0" alt="" title="{$LNG.al_save}">
		{if $canKick}<a href="game.php?page=alliance&amp;mode=admin&amp;action=membersKick&amp;id={$userID}" onclick="return confirm('{$memberListRow.kickQuestion}');" style="border: 1px solid #212121;vertical-align:top;width:16px;height:16px;display:inline-block;margin:2px;"><img src="{$themePath}pic/abort.gif" border="0" alt=""></a>{/if}{else}-{/if}
		</td>
	</tr>
	{/foreach}
	</tbody>
