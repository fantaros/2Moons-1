<div id="leftmenu">
	<ul id="menu">
		...
		{if isModulAvalible($smarty.const.MODULE_MESSAGES)}<li><a href="game.php?page=messages">{$LNG.lm_messages}{if $new_message > 0}<span id="newmes"> (<span id="newmesnum">{$new_message}</span>)</span>{/if}</a></li>{/if}
		{if isModulAvalible($smarty.const.MODULE_NOTICE)}<li><a href="javascript:OpenPopup('game.php?page=notes', 'notes', 720, 300);">{$LNG.lm_notes}</a></li>{/if}
